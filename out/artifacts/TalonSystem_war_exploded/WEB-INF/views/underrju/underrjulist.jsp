...
<div class="am-pagebody">
	<div class="card pd-20 pd-sm-40">
		...
		<a href="<c:url value='/underrju/newunderrju'/>"
		...
		<div class="table-responsive">
			<table id="datatable1" class="table table-hover table-bordered table-primary mg-b-0">
				...
				<tbody>
					<c:forEach items="${underrjus}" var="underrju">


						<tr>
							<td>${underrju.callname}</td>
							<td>${underrju.rju.callname}</td>
							<sec:authorize access="hasRole('DBA')">
								<td><a
									href="<c:url value='/underrju/edit-underrju-${underrju.id}' />"
									class="btn btn-primary btn-icon"><div>
											<i class="fa fa-edit"></i>
										</div></a></td>
								<td><a
									href="<c:url value='/underrju/delete-underrju-${underrju.id}' />"
					...
					<ul class="pagination pagination-basic mg-b-0">
						...
						<c:if test="${currentPage != 1}">
							<li class="page-item"><a class="page-link"
								href="<c:url value='/underrju/list?page=${currentPage - 1}' />"
    ...
    The when condition does not display a link for the current page--%>
						...
						<c:forEach begin="1" end="${noOfPages}" var="i">
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="<c:url value='/underrju/list?page=${i}' />">${i}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
