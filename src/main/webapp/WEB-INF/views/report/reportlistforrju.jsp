...
<div class="am-pagebody">
	<div class="card pd-20 pd-sm-40">
		...
		<h6 class="card-body-title">о причинах изъятых
			талонов-предупреждения № 1, 2, 3 по РЖУ АО "УТЙ"</h6>
		<h6>${datecriteria}</h6>
					...
					<c:forEach var="entry" items="${report}">
						...
						<tr>
							...
							<c:forEach var="nestentry" items="${entry.value}">
								...
								<c:set var="horTotal" value="${horTotal+nestentry.value}"
									scope="page" />
							</c:forEach>
							<c:choose>
								<c:when test="${horTotal!=0}">
									<td
										style="background-color: gold; font-family: serif; font-weight: bold;">${horTotal}</td>
								</c:when>
								<c:otherwise>
									<td
...
<sec:authorize access="hasRole('DBA') or hasRole('ADMIN')">
	<jsp:include page="repcriteriaforrju.jsp"></jsp:include>
</sec:authorize>
