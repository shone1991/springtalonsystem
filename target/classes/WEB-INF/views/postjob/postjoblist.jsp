				...
				<tbody>
					<c:forEach items="${jobs}" var="job">
						...
						<tr>
							...
							<sec:authorize access="hasRole('DBA')">
								<td><a
									href="<c:url value='/job/edit-job-${job.id}' />"
									class="btn btn-primary btn-icon"><div>
											<i class="fa fa-edit"></i>
										</div></a></td>
								<td><a
									href="<c:url value='/job/delete-job-${job.postname}' />"
					...
					<ul class="pagination pagination-basic mg-b-0">
						...
						<c:if test="${currentPage != 1}">
							<li class="page-item"><a class="page-link"
								href="<c:url value='/job/list?page=${currentPage - 1}' />"
    ...
    The when condition does not display a link for the current page--%>
						...
						<c:forEach begin="${currentPage}" end="${currentPage+5}" var="i">
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="<c:url value='/job/list?page=${i}' />">${i}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
