...
<div class="am-pagebody">
	<div class="card pd-20 pd-sm-40">
		...
		<a href="<c:url value='/violguide/newviolguide'/>"
		...
		<div class="table-responsive">
			<table id="datatable1" class="table table-hover table-bordered table-primary mg-b-0">
				...
				<tbody>
					<c:forEach items="${violguides}" var="violguide">


						<tr>
							<td>${violguide.contentViol}</td>
							<td>${violguide.elements}</td>
							<td>${violguide.conclviolguide.content}</td>
							<sec:authorize access="hasRole('DBA')">
								<td><a
									href="<c:url value='/violguide/edit-violguide-${violguide.id}' />"
									class="btn btn-primary btn-icon"><div>
											<i class="fa fa-edit"></i>
										</div></a></td>
								<td><a
									href="<c:url value='/violguide/delete-violguide-${violguide.id}' />"
					...
					<ul class="pagination pagination-basic mg-b-0">
						...
						<c:if test="${currentPage != 1}">
							<li class="page-item"><a class="page-link"
								href="<c:url value='/violguide/list?page=${currentPage - 1}' />"
    ...
    The when condition does not display a link for the current page--%>
						...
						<c:forEach begin="1" end="${noOfPages}" var="i">
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="<c:url value='/violguide/list?page=${i}' />">${i}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
